...
install_vault() {
  ...
  local zip_file="vault_${VAULT_CURRENT_VERSION}_linux_${arch}.zip"

  id vault &>/dev/null || useradd --system --home "${VAULT_DATA_PATH}" --shell /sbin/nologin vault
  curl -O --continue-at - "https://releases.hashicorp.com/vault/${VAULT_CURRENT_VERSION}/${zip_file}"
  echo -e "\n"
  unzip -o "${zip_file}"
  rm -f "${zip_file}"
  ...
  systemctl status vault || true
  echo -e '\n✅ Vault has been successfully installed:
...
setup_autocomplete() {
  ...
  if ! grep -q 'complete -C .*/vault vault' "$completion_file" 2>/dev/null; then
      echo "complete -C $VAULT_BIN_PATH/vault vault" | tee -a "$completion_file" > /dev/null
      echo "export VAULT_ADDR=$VAULT_ADDR" | tee -a "$completion_file" > /dev/null
      echo -e "\n✅ Autocomplete added to $completion_file"
  else
      echo -e "\n✅ Autocomplete has already been added to $completion_file"
  fi
}
...
run_based_on_distro() {
  ...
  if [[ "$distro" =~ ubuntu || "$distro" =~ debian ]]; then
    ...
    check_requirements
    install_vault
    setup_autocomplete "$VAULT_COMPLETION_DEB"

  elif [[ "$distro" =~ centos || "$distro" =~ rhel || "$distro" =~ fedora ]]; then
    echo -e "\n✅ RedHat family distribution. Install Vault...\n"
    check_requirements
    install_vault
    setup_autocomplete "$VAULT_COMPLETION_RPM"
